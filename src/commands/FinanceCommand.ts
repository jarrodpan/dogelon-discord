		...
		return Promise.resolve()
				...
				try {
					response = await axios.get(
						'https://query1.finance.yahoo.com/v10/finance/quoteSummary/' +
							ticker +
							'?region=AU&lang=en-AU&corsDomain=au.finance.yahoo.com&formatted=true&modules=price%2CsummaryDetail%2CpageViews%2CfinancialsTemplate'
					);
			...
			.then(([response, data, error]) => {
				...
				if (!error) {
					...
					const priceChange =
						result.currencySymbol + result.regularMarketChange.fmt; // BUG: fix this line if the response is messed up
					const pcChange = result.regularMarketChangePercent.fmt;
					const footer =
						result.exchangeName +
						'  •  ' +
						result.quoteSourceName +
						' ' +
						result.currency;
				...
				if (embed == null)
					throw new Error(
						'FinanceCommands: embed is undefined or null'
					);
