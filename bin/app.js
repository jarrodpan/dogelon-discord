...
client.on('messageCreate', (message) => {
    ...
    let action = new Action_1.Action(message, "", async (_) => { return "ligma balls"; });
    if (ligma > -1)
        queue.push(action);
    // tokenize
    ...
    tokens.forEach(token => {
        if (token.charAt(0) == "$")
            queue.push(new Action_1.Action(message, token, async (token) => {
                // code goes here
        ...
        if (token.slice(0, 2) == "r/" || token.slice(0, 3) == "/r/")
            queue.push(new Action_1.Action(message, token, async (token) => {
